...
#define MAX_ORDER_ALLOWED 22 // Max 4 MB (was 20 = 1 MB)
...
{
    if (!memoryStart || memoryBytes < MIN_BLOCK_SIZE)
        return;
    ...
    maxOrder = 0;
    while (order_size(maxOrder) < poolSize)
        maxOrder++;
...
static uint32_t size_to_order(uint32_t userSize)
{
    ...
    uint32_t ord = 0;
    while (order_size(ord) < need)
        ord++;
    /* never below MIN_BLOCK_SIZE */
    if (order_size(ord) < MIN_BLOCK_SIZE)
        ord = 5;
    return ord;
}
...
static int alloc_rec(uint32_t idx, uint32_t order, uint32_t wantOrder, uint32_t *outOffset)
{
    ...
    if (alloc_rec(leftIdx, childOrd, wantOrder, outOffset))
        return 1;
    if (alloc_rec(rightIdx, childOrd, wantOrder, outOffset))
        return 1;
...
void freeMemory(void *ptr)
{
    ...
    uint32_t blockSize = order_size(order) - HDR_SIZE;
...
// No debug counters in clean build
