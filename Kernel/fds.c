...
int initFileDescriptors() {
    ...
    fileDescriptors[STDIN].resource = allocMemory(sizeof(Stream));
...
int readFromFD(int fd, char *buf, int count) {
    ...
    if (fd == STDIN && getCurrentProcess()->foreground == 0) {
        char * message = "Error: STDIN cannot be read in background processes.\n";
        printToFd(fd, message, strlen(message));
        return 0;
    }
    ...
    while(sizeRead < count) {
        ...
        if (!stream->eof || (fd != STDIN && (stream->writers != 0 || stream->dataAvailable > 0))){
            ...
            // Si leímos un newline, podemos terminar early para líneas completas (chequear si es necesario)
            if (lastRead == '\n') {
                break;
            }
...
//se podria ver de optimizar para escribir bloques grandes
int writeToFD(int fd, const char *buf, int count, unsigned long hexColor) {
    ...
    if (fd == STDOUT || fd == STDERR)
    {
        ...
        {
            toPrint[0] = buf[written]; //raro,  no se podria todo en una sola linea
            //vdPrint(toPrint, color);
            printToFd(fd, toPrint, 1);
        }
        return written;
    }
...
int closeFD(int fd) {
   ...
   if (fd < 0 || fd == STDOUT || fd == STDERR || fd >= MAX_FDS || !fileDescriptors[fd].isOpen)
    ...
    //no hay escritor y quedan lectores. borrar todo y avisar EOF a los lectores. pues ya no habra mas datos
    ...
    //cada proceso es encargado de cerrar sus fds abiertos al morir
}
