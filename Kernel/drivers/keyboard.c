...
void handleCtrlD()
{
    ...
    {
        // Buffer no vacío: agregar \n para enviar el contenido actual, luego EOF
        addCharToBuffer(NEW_LINE_CHAR, 1); // Mostrar el \n
        addCharToBuffer(EOF, 0);           // Agregar EOF después del newline

        // Notificar al semáforo dos veces (una por el \n y otra por el EOF)
        if (keyboard_semaphore != -1)
        {
            semPost(keyboard_semaphore);
            semPost(keyboard_semaphore);
