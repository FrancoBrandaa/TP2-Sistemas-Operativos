...
static PID blocked_reader_pid = -1; // PID of process waiting for keyboard input
...
void addCharToBuffer(int8_t ascii, uint8_t showOutput)
{
    if (ascii != TABULATOR_CHAR)
    {
        ...
        // If a process is blocked waiting for input, check if we should unblock it
        if (blocked_reader_pid != -1)
        {
            int should_unblock = 0;

            // Case 1: Not waiting for return key - any character unblocks
            if (!(keyboard_options & AWAIT_RETURN_KEY))
            {
                should_unblock = 1;
            }
            // Case 2: Waiting for return key - only newline or EOF unblocks
            else if (ascii == NEW_LINE_CHAR || ascii == EOF)
            {
                should_unblock = 1;
            }
            ...
            if (should_unblock)
            {
                unblockProcess(blocked_reader_pid);
                // Don't clear blocked_reader_pid here - let getKeyboardCharacter do it
            }
        }
        return;
    }
...
int8_t getKeyboardCharacter(enum KEYBOARD_OPTIONS ops)
{
    ...
    int need_to_wait = (to_write == to_read) ||
    ...
    if (need_to_wait)
    {
        // Block the process ONCE before entering the wait loop
        blocked_reader_pid = getpid();
        blockProcess(blocked_reader_pid);

        // Continue halting until condition is met
        // (will be unblocked by keyboard interrupt when data arrives)
    ...
    keyboard_options = 0;
    blocked_reader_pid = -1; // Clear after reading
